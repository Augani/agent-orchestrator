...
def activity_key(item: dict) -> tuple:
    return (bool(item.get("attention_required")), bool(item.get("active")), item.get("updated_at") or "")
...
class DashboardState:
    ...
    def changed_files(self, workspace: str, baseline: dict) -> list[str] | None:
        with self.lock:
            ...
            if not cached or time.monotonic() - cached[0] >= 2:
                ...
                except (jobs.RunnerError, OSError, TimeoutError, jobs.subprocess.TimeoutExpired):
        ...
        if not current.get("is_git"):
            return None
        return sorted(jobs.status_paths(current.get("status", [])) - jobs.status_paths(baseline.get("status", [])))
    ...
    def job(self, path: Path) -> tuple[dict, dict]:
        ...
        row = {
            ...
            "group": status.get("group"), "plan_id": status.get("plan_id"),
            "checklist_item": status.get("checklist_item"), "state": state,
            "execution_state": status.get("execution_state"), "review_state": status.get("review_state"),
            "phase": status.get("current_phase"), "elapsed_seconds": jobs.elapsed_seconds(status),
            "created_at": status.get("created_at"), "updated_at": latest.get("at") or status.get("finished_at") or status.get("created_at"),
            "recent_message": progress[-1].get("message") if progress else None,
            "current_work": progress[-1].get("message") if progress else status.get("role"),
            "changed_files": len(files) if files is not None else None,
            "usage": status.get("usage_summary"), "pending_questions": questions,
    ...
    def overview(self) -> dict:
        ...
        for record in jobs.read_feedback():
            try:
                ...
                project["updated_at"] = max(project["updated_at"], record.get("answered_at") or record["created_at"])
                if record.get("state") == "pending":
                    project["pending_feedback"].append({**feedback_public(record), "project_id": project["id"]})
        ...
        for path in sorted(jobs.plans_dir().glob("*/plan.json")):
            try:
                ...
                project["plans"].append({key: plan.get(key) for key in ("plan_id", "title", "counts", "complete")})
                project["updated_at"] = max(project["updated_at"], plan.get("updated_at") or plan.get("created_at") or "")
        ...
        for project in projects.values():
            ...
            project["attention_required"] = bool(project["pending_feedback"]) or any(row["attention_required"] for row in rows)
            project["attention_count"] = len(project["pending_feedback"]) + sum(row["attention_required"] for row in rows)
            project["updated_at"] = max([project["updated_at"], *[row["updated_at"] or "" for row in rows]])
        ...
        return {"projects": sorted(projects.values(), key=activity_key, reverse=True),
                "pending_questions": pending, "updated_at": jobs.utc_now(), "unreadable_records": skipped}

    def project(self, pid: str) -> dict:
        jobs.validate_identifier(pid, "project")
        project = next((item for item in self.overview()["projects"] if item["id"] == pid), None)
        if project is None:
            raise jobs.RunnerError("Unknown project ID")
        project["feedback_history"] = []
        for record in jobs.read_feedback():
            workspace = record.get("workspace")
            if isinstance(workspace, str) and workspace and project_id(workspace) == pid and record.get("state") != "pending":
    ...
    def detail(self, pid: str, job_id: str) -> dict:
        ...
        row, detail = self.job(path)
        events = [{key: event.get(key) for key in ("id", "at", "type", "source", "phase", "message", "question", "state", "verdict")}
                  for event in detail["events"]]
        return {**row, "events": events, "questions": [{key: q.get(key) for key in ("id", "state", "question", "created_at", "answer", "answered_at", "expired_at")} for q in jobs.read_questions(path)],
                "files": detail["files"], "review": detail["status"].get("review"),
                "logs": {"label": "Local / private · last 8 KiB per stream",
                         "stdout": local_tail(path / "stdout.log"), "stderr": local_tail(path / "stderr.log")}}

    def answer(self, parts: list[str], value: str) -> dict:
        if len(parts) == 8 and parts[:2] == ["api", "projects"] and parts[3] == "jobs" and parts[5] == "questions" and parts[7] == "answer":
            self.verify_job(parts[2], parts[4])
            return jobs.answer_worker_question(parts[4], parts[6], value)
        if len(parts) == 6 and parts[:2] == ["api", "projects"] and parts[3] == "feedback" and parts[5] == "answer":
            jobs.validate_identifier(parts[2], "project")
            path = jobs.feedback_dir(parts[4])
            if project_id(jobs.read_json(path / "feedback.json")["workspace"]) != parts[2]:
...
def local_tail(path: Path) -> str:
    try:
        with path.open("rb") as handle:
            ...
            return jobs.ANSI_RE.sub("", handle.read(8192).decode("utf-8", errors="replace"))
    except OSError:
        return ""
...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def respond(self, status: int, body, content_type="application/json; charset=utf-8"):
    ...
    def boundary(self) -> bool:
        ...
        if hosts != [self.server.origin.removeprefix("http://")] or (origins and origins != [self.server.origin]):
    ...
    def authenticated(self) -> bool:
        ...
        if len(tokens) != 1 or not secrets.compare_digest(tokens[0].encode("utf-8"), self.server.token.encode("ascii")):
            self.respond(401, {"error": "Open the dashboard URL printed by dashboard-web to authenticate"})
            return False
        return True
    ...
    def do_GET(self):
        ...
        try:
            ...
            elif len(parts) == 5 and parts[:2] == ["api", "projects"] and parts[3] == "jobs":
            ...
            self.respond(404, {"error": "Local record unavailable or ID does not belong to this project"})
    ...
    def do_POST(self):
        ...
        try:
            ...
            if not isinstance(body, dict) or set(body) != {"answer"} or not isinstance(body["answer"], str) or not body["answer"].strip():
                raise ValueError("Expected only a nonempty answer string")
        except (ValueError, UnicodeError, TimeoutError):
            self.respond(400, {"error": "Expected JSON with one nonempty answer string"})
            return
        try:
            result = self.server.state.answer(self.path.strip("/").split("/"), body["answer"])
            self.respond(200, result)
        except jobs.RunnerError as exc:
            if "not pending" in str(exc):
                self.respond(409, {"error": "This question is no longer pending. Refresh to see its answer."})
            else:
                self.respond(404, {"error": "Unknown ID or record does not belong to this project"})
        except (OSError, KeyError, ValueError):
            self.respond(500, {"error": "Could not save the local answer. Refresh before retrying."})
